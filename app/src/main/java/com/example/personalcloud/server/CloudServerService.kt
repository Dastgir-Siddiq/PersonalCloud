...
class CloudServerService : Service() {
    ...
    private fun startServer(paths: List<String>) {
        ...
        if (wifiLock == null) {
            ...
            // WIFI_MODE_FULL_LOW_LATENCY (4) on Android 12+ gives the best Wi-Fi 6 / 5 GHz
            // throughput by disabling power-save and enabling aggressive OFDMA scheduling.
            // Fall back to FULL_HIGH_PERF (3) on older firmware.
            @Suppress("DEPRECATION")
            val lockMode = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)
                WifiManager.WIFI_MODE_FULL_LOW_LATENCY   // = 4
            else
                WifiManager.WIFI_MODE_FULL_HIGH_PERF     // = 3
            wifiLock = wifiManager.createWifiLock(lockMode, "PersonalCloud::LowLatencyWifiLock")
            wifiLock?.acquire()
        }
