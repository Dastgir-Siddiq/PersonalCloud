...
class FileServer(private val context: Context, private val port: Int = 8080) {
    ...
    companion object {
        ...
        // 2× CPU cores: enough async I/O workers without excess context-switching
        private val WORKER_THREADS = Runtime.getRuntime().availableProcessors() * 2

        // 4 MB TCP send buffer — fills the Wi-Fi 6 air-time window on 5 GHz
        private const val SO_SNDBUF = 4 * 1024 * 1024
        // 2 MB TCP receive buffer — gives ACKs headroom to pipeline
        private const val SO_RCVBUF = 2 * 1024 * 1024
        // Netty write-buffer watermarks: don't stall the pipeline before
        // the kernel drains the socket send-buffer.
        private val WB_WATER_MARK = WriteBufferWaterMark(512 * 1024, 4 * 1024 * 1024)
    }
    ...
    fun start() {
        ...
        server = embeddedServer(Netty, port = port, configure = {
            ...
            // ── Thread pool: 2× CPU count for async NIO ──────────────────────
            workerGroupSize = WORKER_THREADS
            callGroupSize   = WORKER_THREADS
            // ── Per-connection TCP socket options ─────────────────────────────
            // configureBootstrap applies childOption to every accepted socket.
            //  • SO_SNDBUF  4 MB — keeps the Wi-Fi 6 transmit window saturated
            //  • SO_RCVBUF  2 MB — enough head-room for pipelined ACKs
            //  • TCP_NODELAY     — disables Nagle, critical for 5 GHz latency
            //  • SO_KEEPALIVE    — detect dead connections early
            //  • WriteBufferWaterMark — prevents Netty back-pressure stalls
            configureBootstrap = {
                childOption(ChannelOption.SO_SNDBUF,              SO_SNDBUF)
                childOption(ChannelOption.SO_RCVBUF,              SO_RCVBUF)
                childOption(ChannelOption.TCP_NODELAY,            true)
                childOption(ChannelOption.SO_KEEPALIVE,           true)
                childOption(ChannelOption.WRITE_BUFFER_WATER_MARK, WB_WATER_MARK)
            }
        }) {
            install(CORS) {
                            ...
                            fun addFileToZip(file: File, parentPath: String) {
                                if (file.isDirectory) {
                                    ...
                                    try {
                                        ...
                                        file.inputStream().use { it.copyTo(zos, 1024 * 1024) }  // 1 MB buffer
                                        zos.closeEntry()
                    ...
                    try {
                        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                            call.receiveChannel().toInputStream().use { input ->
                                destFile.outputStream().buffered(1024 * 1024).use { output ->
                                    val buffer = ByteArray(1024 * 1024)  // 1 MB — one syscall per Wi-Fi 6 burst
                                    var bytesRead: Int
